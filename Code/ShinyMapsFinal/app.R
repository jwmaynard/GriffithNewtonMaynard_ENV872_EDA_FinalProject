...
MPA.data <- read.csv('https://raw.githubusercontent.com/jwmaynard/GriffithNewtonMaynard_ENV872_EDA_FinalProject/main/DataRaw/NOAA_Marine_Protected_Areas_Inventory_2023.csv',
                     stringsAsFactors = TRUE, 
                     colClasses = c('Year.Established' = 'factor'))
...
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "650"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "643"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "625"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "635"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "641"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "632"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "631"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "319"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "324"] <- "Pacific"


MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "767"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "765"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "534"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "764"] <- "Pacific"
MPA.data.sf$Regions[MPA.data.sf$OBJECTID == "624"] <- "Pacific"


MPA.data.sf <- MPA.data.sf[MPA.data.sf$OBJECTID!="979",]
...
ui <- fluidPage(
  ...
  absolutePanel(bottom = 150, left = 10,
                selectInput("regions", label = "Select region to visualize", choices = unique(MPA.data.sf$Regions), selected = "Northeast"),
                selectInput("filter", 
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    m <- mapview(filter(MPA.data.sf, Regions == regionSelection), zcol = filterSelection, layer.name= c(filterSelection)) 
